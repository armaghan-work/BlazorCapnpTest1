...
window.Radzen = {
    ...
    createSlider: function (slider, parent, range, minHandle, maxHandle, min, max, value) {
        slider.mouseMoveHandler = function (e) {
            var percent = (minHandle == e.target || maxHandle == e.target ? e.target.offsetLeft + e.offsetX : e.offsetX) / parent.offsetWidth;
            var newValue = percent * max;
            var oldValue = range ? value[slider.isMin ? 0 : 1] : value;
            if (slider.canChange && newValue >= min && newValue <= max && newValue != oldValue) {
                slider.invokeMethodAsync('RadzenSlider.OnValueChange', newValue, slider.isMin, percent);
            }
        }
        ...
        slider.mouseDownHandler = function (e) {
            if (minHandle == e.target || maxHandle == e.target) {
                ...
                var percent = e.offsetX / parent.offsetWidth;
                var newValue = percent * max;
                var oldValue = range ? value[slider.isMin ? 0 : 1] : value;
        ...
        return { x: (range ? value[0] : value) / parent.offsetWidth, y: (range ? value[1] : value) / parent.offsetWidth}
    },
